<template>
  ...
  <div class="login_container">
    ...
    <div class="login_box">
      ...
      <!--表单 2个输入框，2个按钮-->
      <el-form label-width="0px" ref="userFormRef" :model="user_form" :rules="userRules" class="login_form">
        <!--clearable可清空  用户名 prop属性校验字段名-->
        <el-form-item prop="username">
          <el-input prefix-icon="el-icon-user" clearable v-model="user_form.username">
          </el-input>
        </el-form-item>
        <!--show-password密码框  密码 prop属性校验字段名-->
        <el-form-item prop="password">
          <el-input prefix-icon="el-icon-lock" v-model="user_form.password" show-password>
          </el-input>
        </el-form-item>
        <!--按钮-->
  ...
  methods: {
    login() {
      ...
      this.$refs.userFormRef.validate(async (valid) => {
        ...
        const { data: res } = await this.$http.post('login', this.user_form) // 接收通过post方式传输过来的用户名、密码
        // 判断
        if (res.code != 200) return this.$message.error(res.msg) // 如果状态码不是200,返回提示信息  --拿到后端编写的提示信息
        window.sessionStorage.setItem('token', res.data.username) // 设置token
