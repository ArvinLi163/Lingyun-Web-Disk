      ...
      <el-table :data="musicList" border stripe>
        ...
        <el-table-column label="文件">
          <template slot-scope="scope">
            ...
            <div v-else>
              <i class="el-icon-headset" style="color: red; font-size: 200%"></i>
              {{ scope.row.fileName }}
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <!-- 修改 -->
            <el-button style="margin-left: 2px" @click="alterFile(scope.row.filePath)" v-model="scope.row.filePath"
            ...
            <!-- 收藏 -->
            <el-button style="margin-left: 2px" v-model="scope.row.id" type="warning" icon="el-icon-star-off"
              size="mini"></el-button>
            <!-- 下载 -->
...
export default {
  data() {
    return {
      ...
      musicList: [],
      filePathList: [],
      filepath: '/' + window.sessionStorage.getItem('token'), // /用户名
      total: 0
    }
  },
  created() {
  ...
  methods: {
    async getMusicList() {
      const { data: res } = await this.$http.get('musicLists?filepath=' + this.filepath)
