      ...
      <el-table :data="musicList" border stripe>
        ...
        <el-table-column label="文件">
          <template slot-scope="scope">
            ...
            <div v-else>
              <i class="el-icon-collection-tag" style="color: red; font-size: 200%"></i>
              {{ scope.row.fileName }}
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <!-- 收藏 -->
            <el-button style="margin-left: 2px" v-model="scope.row.id" type="danger" icon="el-icon-star-off"
              size="mini"></el-button>
            <!-- 下载 v-if="scope.row.isDirectory != 1"判断是否为目录-->
...
export default {
  ...
  methods: {
    async getMusicList() {
      const { data: res } = await this.$http.get('collectionListsFiles?filepath=' + this.filepath)
